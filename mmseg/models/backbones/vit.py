...
import torch.nn.functional as F # for maskclip
...
class TransformerEncoderLayerMaskclip(TransformerEncoderLayer):
    ...
    def forward(self, x, return_qkv=False):
        ...
        def _inner_forward(x, return_qkv=False):
            ...
            if return_qkv:
                ...
                v += x
                v = self.ffn(self.norm2(v), identity=v) # for maskclip
                ##########
                # x = self.attn(self.norm1(x), identity=x)
                # x = self.ffn(self.norm2(x), identity=x)
                # return x, x, x, x
            ...
            x = self.attn(self.norm1(x), identity=x)
            x = self.ffn(self.norm2(x), identity=x)
            return x, q, k, v # for maskclip

        if self.with_cp and x.requires_grad:
            x, q, k, v = cp.checkpoint(_inner_forward,(x, return_qkv))
        else:
            x, q, k, v = _inner_forward(x, return_qkv)
        return x, q, k, v
...
class VisionTransformerMaskclip(BaseModule):
    ...
    def __init__(self,
        ...
        self.patch_embed = PatchEmbed(
            ...
            stride=patch_size,
            padding='corner',
            bias=patch_bias,        # for maskclip(patch_bias)
            norm_cfg=norm_cfg if patch_norm else None,
            init_cfg=None,
        )
    ...
    def forward(self, inputs):
        ...
        outs = []
        for i, layer in enumerate(self.layers):
            # for maskclip(original mmseg : layer(x))
            ...
            if i in self.out_indices:
                ...
                if self.return_qkv[i]:
                    ...
                    v = v.reshape(B, hw_shape[0], hw_shape[1],
                                  C).permute(0, 3, 1, 2).contiguous()
                    out = [out, q, k, v]
                    """v is the feature of the maskclip, minus attention from the last transformer layer.
                        out[0] converts the output of the last transformer layer to 2d.
                        out[1] is cls token output"""
                outs.append(out)
